...
namespace ns3
{
  ...
  {
    static TypeId tid =
        TypeId("ns3::ndn::SftmClient").SetParent<DashMrbart>().AddConstructor<
            SftmClient>();
    return tid;
  }
      ...
      Time & delay)
  {
    ...
    double ts_ns = m_segment_id * msd; // Timestamp of the next segment
    double ts_o = m_player->m_framesPlayed * MPEG_TIME_BETWEEN_FRAMES
